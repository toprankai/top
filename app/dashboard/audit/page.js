  ...
  return (
    <div className="space-y-5 sm:space-y-6 max-w-7xl mx-auto min-w-0">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">Business Audit Reports</h1>
          <p className="text-sm sm:text-base text-slate-500 font-semibold mt-1">Select a business to view its full optimization audit</p>
        </div>
      </div>
      ...
      {filteredProjects.length === 0 ? (
        ...
        <div className="grid gap-4 sm:gap-6 md:grid-cols-2 lg:grid-cols-3 min-w-0">
          {filteredProjects.map((project) => (
            <Card key={project.id} className="group hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-300 border-slate-200 rounded-2xl overflow-hidden flex flex-col bg-white min-w-0">
              <CardHeader className="pb-4 pt-5 px-4 sm:px-6">
                ...
                <CardTitle className="text-xl font-black text-slate-900 line-clamp-1 break-words group-hover:text-blue-600 transition-colors">
              ...
              <CardContent className="px-4 sm:px-6 pb-5 sm:pb-6 pt-0 flex-1 flex flex-col justify-end">
