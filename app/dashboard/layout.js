...
export default function DashboardLayout({ children }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56">
                <DropdownMenuItem asChild>
                  <Link href="/dashboard/settings" onClick={closeSidebarOnMobile}>Settings</Link>
                </DropdownMenuItem>
                <DropdownMenuSeparator />
      ...
      {/* Main content */}
      <div className={`min-h-screen flex flex-col min-w-0 transition-all duration-300 ${sidebarOpen ? 'lg:pl-64' : 'lg:pl-20 group-hover:lg:pl-64'}`}>
        {/* Mobile header */}
        ...
        <main className="flex-1 flex flex-col min-h-0 min-w-0">
          <PlanStatusBanner />
          <div className={`${pathname.includes('/scans/') ? 'p-0' : 'p-4 md:p-6'} flex-1 flex flex-col min-h-0 min-w-0 relative ${pathname.includes('/scans/') ? 'overflow-hidden' : 'overflow-y-auto'}`}>
            {children}
          </div>
        </main>
      </div>
