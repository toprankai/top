...
export default function SEOOSReportsPage() {
  ...
  return (
    <div className="space-y-6 sm:space-y-8 max-w-7xl mx-auto min-w-0 overflow-x-hidden">
      <div>
        <h1 className="text-2xl sm:text-3xl font-black text-slate-900 flex items-center gap-2">
          <BarChart3 className="w-7 h-7 text-blue-600" /> SEOOS Reports
        </h1>
        <p className="text-sm sm:text-base text-slate-500 mt-1">Portfolio-level SEO performance and team throughput</p>
      </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[680px] md:min-w-0 table-fixed text-sm">
                <thead>
                ...
                <tbody className="divide-y divide-slate-50">
                  {data.latestAudits.map(a => (
                    <tr key={a.id} className="hover:bg-slate-50">
                      <td className="py-3 px-3 font-medium text-slate-700 truncate">{a.url}</td>
