...
export default function ReportsPage() {
  ...
  return (
    <div className="space-y-5 sm:space-y-6 max-w-7xl mx-auto min-w-0">
      <div>
        <h1 className="text-2xl sm:text-3xl font-black text-slate-900">Reports</h1>
        <p className="text-sm sm:text-base text-slate-600 mt-1 font-medium">View and analyze all your completed heatmap scans</p>
      </div>
      ...
      {reports.length === 0 ? (
        ...
        <div className="grid gap-4 sm:gap-5 md:grid-cols-2 lg:grid-cols-3 min-w-0">
          {reports.map((report) => (
            <Card key={report.id} className="hover:border-blue-300 transition-all hover:shadow-md flex flex-col group min-w-0">
              <CardHeader className="pb-4 px-4 sm:px-6 pt-5 sm:pt-6">
                ...
                <CardTitle className="text-xl font-bold text-slate-900 line-clamp-1 break-words" title={report.keyword}>
                  "{report.keyword}"
                </CardTitle>
                <CardDescription className="font-semibold text-blue-600 line-clamp-1 break-words flex items-center gap-1.5" title={report.projectName}>
                  <FileText className="w-3.5 h-3.5" /> {report.projectName}
                </CardDescription>
              </CardHeader>
              
              <CardContent className="pt-0 px-4 sm:px-6 pb-5 sm:pb-6 flex-1 flex flex-col justify-end">
