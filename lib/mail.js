...
async function getEmailSettings() {
  try {
    ...
    return {
      appName: settingsMap.branding?.appName || 'TopRank AI',
    ...
    return {
      ...
      from: '"TopRank AI" <noreply@ringscale.ai>'
    }
  }
}
...
export async function sendOTPEmail(email, otp) {
  const mailOptions = {
    from: process.env.EMAIL_FROM || '"TopRank AI" <noreply@ringscale.ai>',
    to: email,
    subject: `Verify your account - ${otp}`,
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 12px; background-color: #ffffff;">
        <div style="text-align: center; margin-bottom: 30px;">
          <h1 style="color: #2563eb; margin: 0; font-size: 24px; font-weight: 800;">TopRank AI</h1>
        ...
        <div style="margin-top: 30px; text-align: center; border-top: 1px solid #e2e8f0; padding-top: 20px;">
          ...
          <p style="color: #94a3b8; font-size: 12px;">&copy; ${new Date().getFullYear()} TopRank AI. All rights reserved.</p>
        </div>
      </div>
    `,
  }
  ...
  const mailOptions = {
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 12px; background-color: #ffffff;">
        ...
        <div style="margin-top: 30px; text-align: center; border-top: 1px solid #e2e8f0; padding-top: 20px;">
          <p style="color: #94a3b8; font-size: 12px;">Thank you for choosing TopRank AI. Let's grow your local SEO!</p>
          <p style="color: #94a3b8; font-size: 12px;">&copy; ${new Date().getFullYear()} TopRank AI. All rights reserved.</p>
        </div>
      </div>
    `,
...
export async function sendWelcomeEmail(email, name) {
  ...
  const appName = settings.appName || 'TopRank AI'
  ...
  const bookMeeting =
    ...
    'https://calendly.com/toprankai/30min'
  
  const mailOptions = {
    from: process.env.EMAIL_FROM || `"${appName}" <noreply@toprankai.ai>`,
    to: email,
